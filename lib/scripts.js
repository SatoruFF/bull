...
moveToActive(queue, jobId) {
    ...
    if (queue.limiter) {
        ...
        // Передаем groupKey или флаг группировки
        if (queue.limiter.groupKey === true) {
            args.push('true');                 // ARGV[9]
        } else if (queue.limiter.groupKey) {
            args.push(queue.limiter.groupKey); // ARGV[9]
        } else {
            args.push('');                     // ARGV[9]
        }
        
        // Режим лимитера
        args.push(queue.limiter.mode || 'time'); // ARGV[10]
    }
...
moveToFinishedArgs(
    ...
    const args = [
        ...
        queueKeys[''],                         // ARGV[9]
        queue.settings.lockDuration,          // ARGV[10]
        queue.token,                           // ARGV[11]
        queue.metrics && queue.metrics.maxDataPoints || '', // ARGV[12]
        // Добавляем параметры для rate limiter
        queue.limiter && queue.limiter.groupKey ? 
            (queue.limiter.groupKey === true ? 'true' : queue.limiter.groupKey) : '', // ARGV[13]
        queue.limiter ? (queue.limiter.mode || 'time') : ''  // ARGV[14]
    ];
